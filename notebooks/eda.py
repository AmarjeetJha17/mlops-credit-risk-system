...
def analyze_missing_values(df: pd.DataFrame):
    ...
    missing_df = pd.DataFrame(
        {"Missing_Count": missing_data, "Missing_Percent": missing_pct}
    )
    ...
    sns.barplot(
        x=missing_df["Missing_Percent"].head(20),
...
def main():
    if not os.path.exists(RAW_DATA_PATH):
        logging.error(
            f"Dataset not found at {RAW_DATA_PATH}. Please download it first."
        )
        return
    ...
    logging.info(
        "EDA Complete. Check 'reports/figures' and 'reports/stats' for outputs."
    )
