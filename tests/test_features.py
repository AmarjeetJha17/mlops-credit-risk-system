...
def test_domain_feature_generator():
    ...
    assert np.isnan(transformed_df.loc[1, "DAYS_EMPLOYED"]), (
        "Anomaly 365243 was not replaced with NaN"
    )

    # 2. Test Calculation Logic
    expected_ratio = 10000 / 100000
    assert np.isclose(
        transformed_df.loc[0, "INC_TO_ANNUITY_RATIO"], expected_ratio, atol=1e-4
    )

    # 3. Test Divide by Zero Handling (Should not be infinity)
    assert not np.isinf(transformed_df.loc[1, "INC_TO_ANNUITY_RATIO"]), (
        "Divide by zero resulted in Inf"
    )
    assert not np.isinf(transformed_df.loc[2, "CREDIT_TO_ANNUITY_RATIO"]), (
        "Divide by zero resulted in Inf"
    )
