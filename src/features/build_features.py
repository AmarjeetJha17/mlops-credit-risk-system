...
def get_feature_lists(df: pd.DataFrame):
    ...
    numeric_features = (
        df[features].select_dtypes(include=["int64", "float64"]).columns.tolist()
    )
    numeric_features.extend(domain_cols)

    categorical_features = (
        df[features].select_dtypes(include=["object", "category"]).columns.tolist()
    )
...
def main():
    ...
    shap_df = pd.DataFrame(
        {"feature": clean_feature_names, "shap_importance": mean_shap}
    )
