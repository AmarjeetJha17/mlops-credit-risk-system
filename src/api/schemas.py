...
class LoanApplication(BaseModel):
    ...
    DAYS_EMPLOYED: int = Field(..., description="Days employed (negative values, 365243 for unemployed)")
    DAYS_BIRTH: int = Field(..., description="Client's age in days at the time of application (negative)")
    ...
    FLAG_OWN_REALTY: str = Field(default="Y", description="Does the client own a house?")

class PredictionResponse(BaseModel):
    prediction: int = Field(..., description="0 = Repaid, 1 = Default")
    probability: float = Field(..., description="Probability of default")
    feature_contributions: Optional[Dict[str, float]] = Field(default=None, description="SHAP values for explainability")
    model_version: str = Field(..., description="Version of the model serving this request")
